...
class MLAdaptationAgent:
    ...
    def calculate_performance_metrics(
        self,
        recent_n_trades: Optional[int] = None
    ) -> PerformanceMetrics:
    ...
    def optimize_parameters(self) -> Dict[str, Any]:
        ...
        for variation in weight_variations:
            ...
            test_params['timeframe_weights'] = {
                k: v * variation 
                for k, v in self.current_parameters['timeframe_weights'].items()
            }
        ...
        improvement = (best_sharpe - baseline_sharpe) / baseline_sharpe if baseline_sharpe != 0 else 0
    ...
    def detect_overfitting(self) -> Dict[str, Any]:
        ...
        sharpe_degradation = (older_metrics.sharpe_ratio - recent_metrics.sharpe_ratio) / older_metrics.sharpe_ratio \
            if older_metrics.sharpe_ratio != 0 else 0
        
        win_rate_degradation = older_metrics.win_rate - recent_metrics.win_rate
        
        overfitting_detected = (
            sharpe_degradation > 0.3 or  # >30% Sharpe degradation
            win_rate_degradation > 0.15   # >15% win rate drop
        )
    ...
    def _calculate_metrics_for_trades(self, trades: List[TradeOutcome]) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            avg_loss=np.mean([abs(t.pnl) for t in trades if t.pnl < 0]) if (total - wins) > 0 else 0,
            profit_factor=1.0,  # Simplified
            sharpe_ratio=self._calculate_sharpe([t.pnl_pct for t in trades]),
            max_drawdown=self._calculate_max_drawdown(trades),
            avg_hold_time=timedelta(seconds=np.mean([t.hold_duration.total_seconds() for t in trades])),
